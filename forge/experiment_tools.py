...
def init_checkpoint(checkpoint_dir, data_config, model_config, resume):
    ...
    if not checkpoint_dir_exists:
        if resume:
            raise ValueError(
                "Can't resume when the checkpoint dir '{}' doesn't exist.".format(
                    checkpoint_dir
                )
            )
        else:
        ...
        raise ValueError(
            "Checkpoint dir '{}' is not a directory.".format(checkpoint_dir)
        )
    ...
    if experiment_folders:
        ...
        if resume:
            raise ValueError(
                "Can't resume since no experiments were run before in checkpoint"
...
def find_model_files(model_dir):
    ...
    model_files = {
        extract_itr_from_modelfile(f): os.path.join(model_dir, f) for f in model_files
    }
    return model_files
...
def load(conf_path, *args, **kwargs):
    ...
    try:
        ...
        raise ValueError(
            "The config file should specify 'load' function but no such function was "
